...
export class EmpresasService {
  ...
  async createWithQueryRunner(
    ...
    const existingRazaoSocial = await repository.findOne({
      where: { razaoSocial: createEmpresaDto.razaoSocial },
    });
    ...
    const existingCnpj = await repository.findOne({
    ...
    if (existingEmail) {
      // Adicionado validação de e-mail da empresa
      throw new ConflictException('Já existe uma empresa com este e-mail.');
    }
    if (createEmpresaDto.sigla) {
      const existingSigla = await repository.findOne({
        where: { sigla: createEmpresaDto.sigla },
      });
      if (existingSigla) {
