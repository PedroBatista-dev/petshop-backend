...
export class UsuariosController {
  ...
  async findOne(@Param('id') id: string, @Request() req) {
    const usuario = await this.usuarioService.findOneById(id, req.user.idEmpresa);
    if (!usuario) {
  ...
  async update(@Param('id') id: string, @Body() updateUsuarioDto: UpdateUsuarioDto, @Request() req) {
    if (req.user.id === id) { 
      return this.usuarioService.update(id, updateUsuarioDto, req.user.idEmpresa);
    }
    ...
    const targetUser = await this.usuarioService.findOneById(id, req.user.idEmpresa);
    if (!targetUser) {
  ...
  async remove(@Param('id') id: string, @Request() req) {
    const userToDelete = await this.usuarioService.findOneById(id, req.user.idEmpresa);
    if (!userToDelete) {
